...
public class UIViewManager: BaseManager<UIViewManager>
{
    ...
    private async Task<UIViewBase> ShowNewView(SO_UIViewConfig config)
    {
        ...
        {
              Debug.LogError("展示新窗口失败，Layer不存在");
              return null; 
        }
    ...
    public Vector2 ConvertWorldPositionToRootCanvasPosition(Vector3 worldPosition)
    {
        ...
        Vector2 pos = main.WorldToScreenPoint(worldPosition);
        
        pos.x = pos.x / main.pixelWidth * screenUICanvasRootSize.x - screenUICanvasRootSize.x * 0.5f;
        pos.y = pos.y / main.pixelHeight * screenUICanvasRootSize.y - screenUICanvasRootSize.y * 0.5f;
        return pos;
    }
